...
func ParseEdit(tokens []string) ([]string, error) {
	...
	textContent, err := LeerContenidoArchivo(contentPath)
	if err != nil {
		return tokens[contador:], err
	}
	fileContent := util.SplitStringChunks(textContent)

	_, err = superblock.EditFile(util.UserOnline.Path, 0, parentsDir, dest, len(fileContent)*64, fileContent)
	if err != nil {
		return tokens[contador:], err
	}

	util.Respuestas = append(util.Respuestas, fmt.Sprintf("Se edito el contenido de %s exitosamente!", dest))
